<template>
	...
	<div>
		<el-row>
		    ...
		    <el-col :span="11">
		  		<div>
		  			<div class="channel-title">已选择 <span @click="cancelAll">撤销全部选择</span></div>
...
<script>
	...
	export default{
		...
		methods:{
		    ...
		    remove(node, data) {   //node是点击的当前节点
		        const parent = node.parent;
		        const children = parent.data.children || parent.data;
		        const index = children.findIndex(d => d.id === data.id);
		        children.splice(index, 1);
		        if (parent.data.children) {
		        	if (parent.data.children.length==0) { //如果父级没有子集的话  父级也需要自动删除
		        	let flagIndex;
		        	if (parent.data.type=="province") {
		        		flagIndex = parent.parent.data.findIndex(d => d.id === parent.data.id);
		        	    parent.parent.data.splice(flagIndex, 1);
		        	}else if(parent.data.type=="city"){		        		
		        		flagIndex = parent.parent.data.children.findIndex(d => d.id === parent.data.id);
		        		parent.parent.data.children.splice(flagIndex, 1);
			        	if (parent.parent.data.children.length==0) {  //如果父级的父级也没有子集
			        			flagIndex = parent.parent.parent.data.findIndex(d => d.id === parent.data.id);
		    ...
		    renderContent(h, { node, data, store }) {
		        return (
		          <span class="test-span">
		            ...
		            <span>
		              <span class="delete-btn" type="text" on-click={ () => this.remove(node, data) }>×</span>

		            </span><span style="display: none;"></span>
		          </span>);
		    },
			toLeft(){
				if (this.tags.length==0) {  //并未勾选新的数据
			...
			selectThis(val, isSelect){  //选中当前节点
				if (isSelect) {
					this.tags.push({
